...
interface TinyIconLinkProps {
  href?: string;
  label: string;
  children: ReactNode;
}

const TinyIconLink = ({ href, label, children }: TinyIconLinkProps) => {
  const className =
    'h-7 w-7 inline-flex items-center justify-center rounded-sm border border-border text-gray-500 hover:text-foreground hover:bg-card transition-colors';

  if (!href) {
    return (
      <span className={`${className} opacity-40 pointer-events-none`} aria-disabled>
        {children}
      </span>
    );
  }
