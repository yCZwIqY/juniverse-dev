...
const LinkButton = ({ href, label, variant, icon }: LinkButtonProps) => {
  const baseClassName =
    'w-full flex items-center justify-center gap-1.5 font-semibold rounded-sm px-3 py-2 text-sm border transition-colors';
  const variantClassName =
    variant === 'primary'
      ? 'bg-accent text-white border-accent hover:opacity-90'
      : 'bg-transparent border-border text-foreground hover:bg-card';
  const disabledClassName = 'opacity-40 cursor-not-allowed pointer-events-none';

  if (!href) {
    return (
      <span className={`${baseClassName} ${variantClassName} ${disabledClassName}`} aria-disabled>
        {icon}
        <span>{label}</span>
      </span>
    );
    ...
    >
      <Link href={detailHref} className={'lg:col-span-6 w-full overflow-hidden rounded-md block'}>
        {thumbnail ? (
          ...
          <div
            className={
              'aspect-video min-h-[260px] w-full bg-card border border-border flex items-center justify-center text-sm text-gray-500 lg:min-h-[320px]'
            }
          >
            NO IMAGE
          </div>
      ...
      <div className={'lg:col-span-4 flex flex-col justify-between gap-3'}>
        ...
        <div className={'flex flex-wrap gap-1 [&_div]:!text-xs'}>
