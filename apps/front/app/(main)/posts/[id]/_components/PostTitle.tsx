...
const PostTitle = ({ post }: PostTitleProps) => {
  ...
  return (
    <div className={'flex flex-col items-start gap-2'}>
      <Tag className={'!text-base lg:hidden'} onClick={() => router.push(`/posts?category=${menu.id}`)}>
        {menu.name}
      </Tag>
      <h1 className={'text-2xl font-bold'}>{title}</h1>
      {subtitle && <h3 className={'text-lg'}>{subtitle}</h3>}
      <div className={'flex flex-col lg:flex-row gap-2 items-start py-2'}>
        <Tag className={'!text-base hidden lg:block'} onClick={() => router.push(`/posts?category=${menu.id}`)}>
          {menu.name}
        </Tag>
        <div className={'flex flex-col lg:flex-row gap-2 lg:items-center text-gray-600'}>
          ...
          <div className={'flex gap-2'}>
            {tags.slice(0, 5).map((tag) => (
              <Tag key={tag} onClick={() => router.push(`/posts?search=${tag}`)}>
                #{tag}
              </Tag>
            ))}
          </div>
